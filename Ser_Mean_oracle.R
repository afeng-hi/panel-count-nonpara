...
Loss=function(theta){
  ...
  len1 = INN1+nknots1-1
  id.xi1   = 1:len1
  id.alpha = (len1+1) : length(theta)
  
  ### 
  Zmat = cbind(Z1, Z2)
  expcoeff = exp(Zmat %*% theta[id.alpha] + 2 * X^2)
...
cal.DE<-function(theta){
  ...
  for(i in which(Delta==1)){for(j in 1:K[i]){
    ...
    gk[id.alpha]=gk[id.alpha]+2*(c(expcoeff[i]*Lambdatheta[rank[i],i,j])-dN[i,j])*c(expcoeff[i]*Lambdatheta[rank[i],i,j])*Zmat[i,]
  }}
  
  for(i in which(Delta==0)){
    if(sur[rank[i]]!=0&&(rank[i]!=mm)){
      temp1=rep(0,len1); temp_alpha=rep(0, length(id.alpha));
      for(j in 1:K[i]){for(m in (rank[i]+1):mm){
        temp1=temp1+(sur[m-1]^Uexpcoeff[i]-sur[m]^Uexpcoeff[i])*2*(c(expcoeff[i]*Lambdatheta[m,i,j])-dN[i,j])*expcoeff[i]*dBB1[m,i,j,]
   
        temp_alpha=temp_alpha+(sur[m-1]^Uexpcoeff[i]-sur[m]^Uexpcoeff[i])*2*(c(expcoeff[i]*Lambdatheta[m,i,j])-dN[i,j])*c(expcoeff[i]*Lambdatheta[m,i,j])*Zmat[i,]
      }}
      if((sur[rank[i]]^Uexpcoeff[i])>0.1^8){
        temp1=temp1/(sur[rank[i]]^Uexpcoeff[i]); gk[id.xi1]=gk[id.xi1]+temp1
        temp_alpha=temp_alpha/(sur[rank[i]]^Uexpcoeff[i]); gk[id.alpha]=gk[id.alpha]+temp_alpha
      }
    }
  }
  return(gk)
...
bots.DE<-function(theta){
  ...
  for(i in which(Delta==1)){for(j in 1:K[i]){
    ...
    gk[id.alpha]=gk[id.alpha]+2*(c(expcoeff[i]*Lambdatheta[bots.rank[i],i,j])-dN[i,j])*c(expcoeff[i]*Lambdatheta[bots.rank[i],i,j])*Zmat[i,]
  }}
    ...
    if(bots.sur[bots.rank[i]]!=0&&(bots.rank[i]!=mm)){
      ...
      for(j in 1:K[i]){
        for(m in (bots.rank[i]+1):mm){
          ...
          temp_alpha=temp_alpha+(bots.sur[m-1]^bots.Uexpcoeff[i]-bots.sur[m]^bots.Uexpcoeff[i])*2*(c(expcoeff[i]*Lambdatheta[m,i,j])-dN[i,j])*c(expcoeff[i]*Lambdatheta[m,i,j])*Zmat[i,]
        }
      }
      if((bots.sur[bots.rank[i]]^bots.Uexpcoeff[i])>0.1^8){
        gk[id.xi1]=gk[id.xi1]+temp1/(bots.sur[bots.rank[i]]^bots.Uexpcoeff[i])
        gk[id.alpha]=gk[id.alpha]+temp_alpha/(bots.sur[bots.rank[i]]^bots.Uexpcoeff[i])
      }
    }
  }
  return(gk)
...
AA=rbind(AA, cbind(matrix(0,num_para,len1),diag(1,num_para)),
         cbind(matrix(0,num_para,len1),diag(-1,num_para)))
