...
Loss=function(theta){
  ...
  expcoeff=exp(Wmat %*% theta[id.para])
...
cal.DE<-function(theta){
  ...
  for(i in which(Delta==0)){
    ...
    if(sur[rank[i]]!=0&&(rank[i]!=mm)){
      ...
      temp1=rep(0,len1); temp_para=rep(0, length(id.para));
      ...
      if((sur[rank[i]]^Uexpcoeff[i])>0.1^8){
        ...
        temp_para=temp_para/(sur[rank[i]]^Uexpcoeff[i]); gk[id.para]=gk[id.para]+temp_para
  ...
  for(i in which(Delta==0)){
    ...
    if(bots.sur[bots.rank[i]]!=0&&(bots.rank[i]!=mm)){
      ...
      if((bots.sur[bots.rank[i]]^bots.Uexpcoeff[i])>0.1^8){
        ...
        gk[id.para]=gk[id.para]+temp_para/(bots.sur[bots.rank[i]]^bots.Uexpcoeff[i])
...
## theta = c(xi1, beta_X, alpha_Z1, alpha_Z2)
...
AA.lambda = cbind(
  D1,
  matrix(0, nrow = len1, ncol = num_para)
)
BB.lambda = rep(0, len1)

## 2) beta_X, alpha_Z1 and alpha_Z2 are at least -5
AA.para.lower = cbind(
  matrix(0, nrow = num_para, ncol = len1),
  diag(1, num_para)
)

## 3) beta_X, alpha_Z1 and alpha_Z2 are at most 5
AA.para.upper = cbind(
  matrix(0, nrow = num_para, ncol = len1),
  diag(-1, num_para)
)
